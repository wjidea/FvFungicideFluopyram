...
#fullPlateDataExclude50 <- fullPlateDataExclude50[!fullPlateDataExclude50$setStrain == "14@MISTJ_D6",]

# unique(factor(fullPlateDataExclude50$setStrain))
# grep("TRUE",unique(paste(fullPlateDataExclude50$hormetic, fullPlateDataExclude50$setStrain)), value = TRUE)
...
### 4) Calculate EC50 for conidia germination assay
```{r import data conidia, echo = FALSE,  message = FALSE}
# Prepare data path
conidiaDataPath <- paste(dataPath,"/conidiaAssay.csv",sep="")
conidiaColNames <- c("Isolate", "Trial", "Rep", "0", "0.01", "0.1", "1", "5",
                     "10", "20", "100")
conidiaData <- read.csv(conidiaDataPath, header = TRUE, 
                        col.names=conidiaColNames)

# gather cols into rows
coniDataFormat <- conidiaData %>% tidyr::gather(Conc, germinate, X0:X100)
concLevels <- c(0, 0.01, 0.1, 1, 5, 10, 20, 100)
for (i in c(1:8)){
  idx <- unique(coniDataFormat$Conc)[i]
  lbl <- concLevels[i]
  coniDataFormat$Conc[coniDataFormat$Conc == idx] <- lbl  
}
coniDataFormat$Conc <- as.numeric(coniDataFormat$Conc)
...
### 5) Calculate EC50 for conidia assay
```{r conida EC50, results = "hide", warning = FALSE, echo = FALSE, cache=TRUE}
# create empty list with colNames
EC50Raw <- data.frame(straiName = character(0), EC50 = numeric(0), 
                      StdErr = numeric(0), Lower = numeric(0), 
                      Upper = numeric(0), pvalue = numeric(0))
...
# need to filter non-significant EC50 estimations for the EC50 dataframe
# level of significance: alpha = 0.05
EC50SumNoFilterConidia <- EC50Sum
...
### 6) Read in meta data and make new tables with location information
...
```{r Histogram EC50 distrubnution (plate), echo=FALSE, fig.width=12}
histPlotPlateA <- ggplot(EC50PlateSum, aes(x=EC50)) + 
  ...
  labs(y="Frequency", x=expression('EC'[50]*' (mg/L)')) +
  theme_classic() +
  ...
  #geom_text(x = 10, y =25, label="A", fontface = "bold", size = 10)
  

histPlotConidiaB <- ggplot(EC50ConidiaSum, aes(x=EC50)) + 
  geom_histogram(aes(y=(..count..)),
                 binwidth=.5, 
                 colour="dark green",
                 fill="white",size=0.9) +
  ...
  theme(axis.text   = element_text(size=14), 
        ...
        axis.line.x = element_line(color="black", size = 0.7),
        axis.line.y = element_line(color="black", size = 0.7)) 
  #geom_text(x = 10, y =25, label="B", fontface = "bold", size = 10)
grid.arrange(histPlotPlateA, histPlotConidiaB, ncol=2)


```


####Figure 2 Growth curve fit (Plate and conidia) 
The four parameter log logistic model was used to fit data from both assay
(a) non-linear regression curves for the mycelial growth inhibition assay, (b)
non linear regression curves for the conidial germination assay.
```{r growth curve fit (plate), echo=FALSE, fig.width=12, warning=FALSE}
par(mfrow = c(1,2))
# good fit strains
selStrain2 <- c("7@Mo1b","7@Mont1", "13@14Fv1", "13@14Fv21")
selStrain2Data <- fullPlateData[fullPlateData$setStrain %in% selStrain2,]
selStrain2Model1 <- drm(growthRate~Conc,
                  format(Strain,trim=TRUE),
                  fct=LL.4(names=c("slope","Lower","Upper","EC50")),
                  data=selStrain2Data)
plot(selStrain2Model1,type = "all",
     broken = TRUE,
     col = TRUE,
     cex.legend = 0.9,
     ...
     cex.axis = 1,
     main = "Mycelial growth inhibition assay",
     legendPos = c(95,0.14),
     ylim = c(0,0.14),
     lwd=2.5,
     legend=FLASE)
selStrain2Model1$parNames[[3]][1:4]


selStrain2 <- c("Mo1b@10", "Mont-1@10", "14Fv1@7", "14Fv21@7")
selStrain2Data <- coniNoNaData[coniNoNaData$setStrain %in% selStrain2,,drop=TRUE]
selStrain2Data <- selStrain2Data[order(selStrain2Data$Isolate),]
#selStrain2Data$Isolate <- factor(selStrain2Data$Isolate)
selStrain2Model2 <- drm(germinate~Conc,
                  format(Isolate,trim=TRUE),
                  fct=LL.4(names=c("slope","Lower","Upper","EC50")),
                  data=selStrain2Data)
plot(selStrain2Model2,type = "all", broken = TRUE, col = TRUE, cex.legend = 0.9,
     #level=selStrain2,
     xlab = expression('Log10 Transformed Fluopyram Concentration (mg/L)'),
     ylab = "Germinated conidia", cex.axis = 1,
     main = "Conidial germination assay", ylim = c(0, 50), xlim = c(0,20))
     # legendPos = c(20,0.04))
     #ylim = c(0,0.14))
...
####Figure 3 hormetic effect figures
```{r Figure 3 Hormetic effect figures, echo=FALSE, fig.width=12, warning=FALSE}
# Hormetic effect figures
par(mfrow = c(1,2))
selStrain1 <- c("10@MIVB_C1", "10@Mo1a_2")
selStrain1Data <- fullPlateData[fullPlateData$setStrain %in% selStrain1,]
selStrain1Model.LL4 <- drm(growthRate~Conc,
                  format(Strain,trim=TRUE),
                  fct=LL.4(names=c("slope","Lower","Upper","EC50")),
                  data=selStrain1Data)
mselect(selStrain1Model.LL4, list(BC.4()))
plot(selStrain1Model.LL4,type = "all",
     broken = TRUE,
     col = TRUE,
     cex.legend = 0.9,
     xlab = expression('Log10 Transformed Fluopyram Concentration (mg/L)'),
     ylab = "Mycelial Growth Rate (cm/d)",
     cex.axis = 1,
     main = "Hormetic effect - fit with LL.4 model",
     legendPos = c(95,0.18),
     ylim = c(0,0.20))

selStrain1Model.BC4 <- drm(growthRate~Conc,
                  format(Strain,trim=TRUE),
                  fct=BC.4(),
                  data=selStrain1Data)
plot(selStrain1Model.BC4,type = "all",
     broken = TRUE,
     col = TRUE,
     cex.legend = 0.9,
     xlab = expression('Log10 Transformed Fluopyram Concentration (mg/L)'),
     ylab = "Mycelial Growth Rate (cm/d)",
     cex.axis = 1,
     main = "Hormetic effect - fit with BC.4 model",
     legendPos = c(95,0.18),
     ylim = c(0,0.20))

# mselect(selStrain1Model.BC4, list(LL.4(), LL.3(), CRS.4a()))
...
20 shared _F. virguliforme_ isolates between two assay to compare the correlation
...
update(segplot(reorder(strainName, PLdiff) ~ PLdiffL + PLdiffU, data = xyPlConiData, 
        ...
        xlab = expression('Mycelial growth assay EC'[50]*' - Conidia germination assay EC'[50] * ' (µg/mL)'),
        ylab = "Strains") +
...
plot(pEC50~cEC50, data=xyPlConiData, xlab=expression('Conidia assay EC'[50]),
     ylab=expression('Mycelial growth assay EC'[50]))
...
text(x=1.5, y=3.5, cex=0.8, label = expression(atop(italic("y = 2.553 + 0.0369x"),
                                         "Spearman correlation p = 0.4")))
# anova(lm(pEC50~cEC50, data=xyPlConiData))
# cor.test(xyPlConiData$pEC50,xyPlConiData$cEC50, method="spearman")
# t.test(xyPlConiData$pEC50, xyPlConiData$cEC50, paired = TRUE,
#        alternative = "greater")
```
...
#### Table 3
```{r Table 3 data prep,results = "hide", warning = FALSE, echo = FALSE, cache=TRUE}

# not renderred caused by possible error in the model selection step.
# therefore a csv file was written in the disk and kable will be generated
# directly from the csv file in the disk.

fullPlateDataExclude50 <- fullPlateDataExclude50[!fullPlateDataExclude50$setStrain == "14@MISTJ_D6",]

# detect if there is a hormetic effect
for (strain in levels(factor(fullPlateDataExclude50$setStrain))) {
  subData.3 <- filter(fullPlateDataExclude50, setStrain == strain)
  cat(strain, "\t")
  subData.m3 <- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=LL.4(),
                  data=subData.3)
  modelTest <- mselect(subData.m3, list(BC.4()))
  # cat(modelTest, "\n")
  if (grepl("BC", rownames(modelTest)[1])) {
    fullPlateDataExclude50$hormetic[fullPlateDataExclude50$setStrain == strain] <- TRUE
  } else {
    fullPlateDataExclude50$hormetic[fullPlateDataExclude50$setStrain == strain] <- FALSE
  }
}

hormeticData <- filter(fullPlateDataExclude50, hormetic == TRUE)
hormeticData <- filter(hormeticData, !(setStrain %in% c("7@ARLE_C2","9@MISTJ-D6")))
# create empty list with colNames
EC50RawHormetic <- data.frame(straiName = character(0), EC50 = numeric(0), 
                      StdErr = numeric(0), Lower = numeric(0),
                      Upper = numeric(0), pvalue = numeric(0))
...
for (strainSet in levels(factor(hormeticData$setStrain)) ){
  # subset data by strain set
  subData <- subset(hormeticData, 
                    subset = hormeticData$setStrain == strainSet)
  subData.m1 <- drm(growthRate ~ Conc, 
                    format(setStrain, trim=TRUE),
                    fct=BC.4(),
                    data=subData)
  ...
  # include pvalue along with EC50 calculation
  ED_table_p <- cbind(ED_table,pvalue = ModelSummary$coefficients[4,4])
  EC50RawHormetic <- rbind(EC50RawHormetic, ED_table_p)
}

colnames(EC50RawHormetic) <- c( "EC50", "StdErr", "Lower", "Upper","pvalue")

# split the row setStrain name into two columns PARSE strain names and sets
# strainCols <- do.call(rbind, strsplit(rownames(EC50Sum), "@"))
# strainHormeticList <- str_split_fixed(rownames(EC50RawHormetic), ":", 2)
strainSplitList <- str_split_fixed(rownames(EC50RawHormetic), "@", 2)
strainSplitList <- cbind(strainSplitList, 
                         str_split_fixed(strainSplitList[,2],":",2))
strainSplitList <- strainSplitList[,c(1,3)]
colnames(strainSplitList) <- c("Set","Strain")
EC50SumHormetic <- cbind(EC50RawHormetic, strainSplitList)
#rownames(EC50SumHormetic) <- paste(EC50SumHormetic$Strain,EC50SumHormetic$Set,sep = "@")
EC50SumHormetic$strainSet <- paste(EC50SumHormetic$Strain,
                                   EC50SumHormetic$Set,sep="@")
EC50LL4Full <- EC50SumFilterPlate
EC50LL4Full$strainSet <- paste(EC50LL4Full$Strain,
                               EC50LL4Full$Set,sep="@")

EC50LL4 <- select(EC50LL4Full, EC50, StdErr, Lower, Upper, strainSet)
EC50BC4 <- select(EC50SumHormetic, EC50, StdErr, Lower, Upper, strainSet)
LL4vsBC4 <- merge(EC50LL4, EC50BC4, by = "strainSet", suffixes = c(".LL4", ".BC4"))
LL4vsBC4$diff <- LL4vsBC4$EC50.BC4 - LL4vsBC4$EC50.LL4
LL4vsBC4 <- LL4vsBC4[order(LL4vsBC4$diff),]
LL4vsBC4 <- cbind(str_split_fixed(LL4vsBC4$strainSet,"@",2), LL4vsBC4)
LL4vsBC4 <- LL4vsBC4 %>% rename(Strain = `1`, Set = `2`)



BC4only <- filter(EC50BC4, !(strainSet %in% LL4vsBC4$strainSet) )
BC4only <- cbind(str_split_fixed(BC4only$strainSet,"@",2), BC4only)
BC4only <- BC4only %>% rename(Strain = `1`, Set = `2`)
# adapt for the format in LL4VsBC4
BC4prep1 <- BC4only %>% select(Strain, Set, strainSet)
BC4nRow <- nrow(BC4prep1)
BC4prep2 <- data_frame(EC50.LL4 = rep(NA, BC4nRow),
                       StdErr.LL4 = rep(NA, BC4nRow),
                       Lower.LL4 = rep(NA, BC4nRow),
                       Upper.LL4= rep(NA, BC4nRow))
BC4prep3 <- BC4only %>% select(EC50,StdErr,Lower,Upper) %>%
  rename(EC50.BC4 = EC50,StdErr.BC4 = StdErr,Lower.BC4 = Lower, Upper.BC4=Upper)
BC4prep4 <- data_frame(diff = rep(NA, BC4nRow))
BC4Add <- cbind(BC4prep1,BC4prep2,BC4prep3,BC4prep4)

LL4vsBC4All <- rbind(LL4vsBC4, BC4Add)
write.csv(LL4vsBC4All, "./Data/LL4vsBC4.csv")
getwd()
summary(LL4vsBC4All$diff)
# t.test(LL4vsBC4Table$EC50.LL4, LL4vsBC4Table$EC50.BC4, 
#        paired = TRUE, alternative = "less")
```

```{r Table3 hormetic effect Table, eval = TRUE, echo=FALSE}
# "LL4vsBC4.csv" file was generated from the block above


LL4vsBC4Table <- read.csv("./Data/LL4vsBC4.csv", row.names = 1)
fullPlateData$Strain <- do.call(str_replace_all, 
                                list(fullPlateData$Strain, "_", "-"))
LL4vsBC4Table$Strain <- do.call(str_replace_all, 
                                list(LL4vsBC4Table$Strain, "_", "-"))

subPlate5Ppm <- fullPlateData[fullPlateData$Conc == 5,]
plateRelInhPerc5Ppm <- subPlate5Ppm %>% group_by(Strain) %>%
  summarise(N = length(relGrowthRate),
            relInhibition5 = mean(relGrowthRate),
            relSE5 = sd(relGrowthRate) / sqrt(N))

subPlate10Ppm <- fullPlateData[fullPlateData$Conc == 10,]
plateRelInhPerc10Ppm <- subPlate10Ppm %>% group_by(Strain) %>%
  summarise(N = length(relGrowthRate),
            relInhibition10 = mean(relGrowthRate),
            relSE10 = sd(relGrowthRate) / sqrt(N))

subPlate50Ppm <- fullPlateData[fullPlateData$Conc == 50,]
plateRelInhPerc <- subPlate50Ppm %>% group_by(Strain) %>%
  summarise(N = length(relGrowthRate),
            relInhibition50 = mean(relGrowthRate),
            relSE50 = sd(relGrowthRate) / sqrt(N))

subPlate100Ppm <- fullPlateData[fullPlateData$Conc == 100,]
plateRelInhPerc100ppm <- subPlate100Ppm %>% group_by(Strain) %>%
  summarise(N = length(relGrowthRate),
            relInhibition100 = mean(relGrowthRate),
            relSE100 = sd(relGrowthRate) / sqrt(N))

LL4vsBC4Table5ppm <- merge(LL4vsBC4Table, plateRelInhPerc5Ppm, by="Strain",
                           all.x=TRUE)
LL4vsBC4Table10ppm <- merge(LL4vsBC4Table5ppm, plateRelInhPerc10Ppm, by="Strain",
                            all.x=TRUE)
LL4vsBC4Table50ppm <- merge(LL4vsBC4Table10ppm, plateRelInhPerc, by="Strain",
                          all.x=TRUE)
newLL4vsBC4Table <- merge(LL4vsBC4Table50ppm, plateRelInhPerc100ppm, by="Strain",
                          all.x=TRUE)
kable(newLL4vsBC4Table)
...
```{r Tables S1 S2 S3, echo=FALSE}
######## 2016-05-28 ###################
# revised Table S1
# mycelial assay
# fullPlateData$Strain <- do.call(str_replace_all, 
#                                 list(fullPlateData$Strain, "_", "-"))
# take all 50ppm isolates
# subPlate50Ppm <- fullPlateData[fullPlateData$Conc == 50,]
# subPlate10Ppm <- fullPlateData[fullPlateData$Conc == 10,]
# subPlate5Ppm <- fullPlateData[fullPlateData$Conc == 5,]
...
# merge EC50 plate and relative inhibition together
ECTablePlate10ppm <- merge(EC50PlateTable, plateRelInhPerc10Ppm, by="Strain",
                          all.y=TRUE)
ECTableDF50ppm <- merge(ECTablePlate10ppm,plateRelInhPerc, by="Strain", all=TRUE)
ECTableDF5ppm <- merge(ECTableDF50ppm, plateRelInhPerc5Ppm, by="Strain", all=TRUE)
# ECTableDF5ppm[ECTableDF5ppm$Strain == "MIVB-B4",]
...
# extract the 20 ppm data
subConidia20Ppm <- coniNoNaData[coniNoNaData$Conc == 20,]
subConidia10Ppm <- coniNoNaData[coniNoNaData$Conc == 10,]
subConidia5Ppm <- coniNoNaData[coniNoNaData$Conc == 5,]

#allConidiaIsolateNames <- unique(coniNoNaData$Isolate)
#coniNoNaData[coniNoNaData$Isolate == "13Fv190",]
# summarize the 20ppm mean and number
conidiaRelInhPerc <- subConidia20Ppm %>% group_by(Isolate) %>%
  summarise(N = length(relGerminate),
            relInhibition20 = mean(relGerminate),
            relSE20 = sd(relGerminate) / sqrt(N))

conidiaRelInhPerc10ppm <- subConidia10Ppm %>% group_by(Isolate) %>%
  summarise(N = length(relGerminate),
            relInhibition10 = mean(relGerminate),
            relSE10 = sd(relGerminate) / sqrt(N))

conidiaRelInhPerc5ppm <- subConidia5Ppm %>% group_by(Isolate) %>%
  summarise(N = length(relGerminate),
            relInhibition5 = mean(relGerminate),
            relSE5 = sd(relGerminate) / sqrt(N))

newECTableConidia20ppm <- merge(EC50ConidialTable, conidiaRelInhPerc,
                             by.x="Strain", by.y="Isolate", all=TRUE)
newECTableConidia10ppm <- merge(newECTableConidia20ppm, conidiaRelInhPerc10ppm,
                                by.x="Strain", by.y="Isolate", all=TRUE)
newECTableConidia5ppm <- merge(newECTableConidia10ppm, conidiaRelInhPerc5ppm,
                                by.x="Strain", by.y="Isolate", all=TRUE)

kable(newECTableConidia5ppm, 
...
#### Figure S1  S2
```{r  reproducibility bar plots, echo=FALSE}
# plate reproduce
dupPlateStrains <- EC50SumFilterPlate[duplicated(EC50SumFilterPlate$Strain),]$Strain
dupPlateData <- EC50SumFilterPlate[EC50SumFilterPlate$Strain %in% dupPlateStrains, ]
# table(factor(dupPlateData$Strain))

dupPlateDataSort <- dupPlateData[order(dupPlateData$EC50), ]
dupPlateDataSort <- dupPlateDataSort[order(dupPlateDataSort$Strain), ]
dupPlateDataSort$Set2 <- rep(1:2, length(dupPlateDataSort$Set)/2)
dupPlateDataSort$set2Strain <- paste(dupPlateDataSort$Strain,dupPlateDataSort$Set2,sep="_")

# if(dupPlateDataSort[1,]$EC50 < dupPlateDataSort[2,]$Upper & 
#   dupPlateDataSort[1,]$EC50 > dupPlateDataSort[2,]$Lower) {
#   cat("TRUE")
#   }

for (i in seq(1,43,2)){
  if (dupPlateDataSort[i,]$Upper > dupPlateDataSort[i+1,]$Lower){
    dupPlateDataSort$reproduce[i:(i+1)] <- TRUE
  } else {
    dupPlateDataSort$reproduce[i:(i+1)] <- FALSE
  }
}
# table(dupPlateDataSort$reproduce)/2

# car::Anova(lme4::lmer(EC50 ~ Set2 + (1|Strain),dupPlateDataSort))

ggplot(dupPlateDataSort, aes(y = EC50, x = set2Strain)) + 
  geom_bar(stat="identity", aes(fill = reproduce)) + 
  geom_errorbar(aes(ymax = Upper, ymin=Lower), 
               position = position_dodge(width = 0.9), width=0.25) +
  labs(x = "Strains") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 5)) +
  scale_fill_manual(values=c("#f7756d","#08ba38"), 
                       name="Reproducibility",
                       breaks=c("TRUE", "FALSE"),
                       labels=c("Yes (n=21)", "No (n=1)"))


# conidia reproduce
dupConidiaStrains <- EC50SumFilterConidia[duplicated(EC50SumFilterConidia$Strain),]$Strain
dupConidiaData <- EC50SumFilterConidia[EC50SumFilterConidia$Strain %in% dupConidiaStrains,]
dupConidiaData$Strain <- factor(dupConidiaData$Strain)

dupConidia2Name <- names(table(dupConidiaData$Strain)[table(dupConidiaData$Strain)<=2])
dupConidia2Data <- dupConidiaData[dupConidiaData$Strain %in% dupConidia2Name,]
dupConidia2DataSort <- dupConidia2Data[order(dupConidia2Data$EC50),]
dupConidia2DataSort <- dupConidia2DataSort[order(dupConidia2DataSort$Strain),]
dupConidia2DataSort$Set2 <- rep(1:2, length(dupConidia2DataSort$EC50)/2)
dupConidia2DataSort$set2Strain <- paste(dupConidia2DataSort$Strain,
                                        dupConidia2DataSort$Set2, sep="_")
for (i in seq(1,101,2)){
  if (dupConidia2DataSort[i,]$Upper > dupConidia2DataSort[i+1,]$Lower){
    dupConidia2DataSort$reproduce[i:(i+1)] <- TRUE
  } else {
    dupConidia2DataSort$reproduce[i:(i+1)] <- FALSE
  }
}
 table(dupConidia2DataSort$reproduce)/2

ggplot(dupConidia2DataSort, aes(y = EC50, x = set2Strain)) + 
  geom_bar(stat="identity", aes(fill = reproduce) ) + 
  geom_errorbar(aes(ymax = Upper, ymin=Lower), 
               position = position_dodge(width = 0.9), width=0.25) +
  labs(x = "strains") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 4.5)) +
    scale_fill_manual(values=c("#f7756d","#08ba38"), 
                       name="Reproducibility",
                       breaks=c("TRUE", "FALSE"),
                       labels=c("Yes (n=40)", "No (n=11)"))

```


```{r model selection, eval = FALSE, echo=FALSE}
fullPlateDataExclude50$Strain <- do.call(str_replace_all, 
                                list(fullPlateDataExclude50$Strain, "_", "-"))
subHormeticData <- 
  fullPlateDataExclude50[fullPlateDataExclude50$Strain %in% LL4vsBC4Table$Strain,]

subData.hormetic <- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=LL.4(),
                  data=subHormeticData)
mselect(subData.hormetic, list(LL.3(), W1.4(), W2.4(), CRS.4c(), BC.4()))

subData.2 <- subset(fullPlateDataExclude50, 
                  subset = fullPlateDataExclude50$setStrain %in% c("2@AL1b","7@Mont1", "13@14Fv1", "13@14Fv21"))
subData.3 <- subset(fullPlateDataExclude50, 
                  subset = fullPlateDataExclude50$setStrain %in% c("14@Mont1"))
subData.m2 <- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=LL.4(),
                  data=subData.2)
summary(subData.m2)
ED(subData.m2, respLev = c(50), type = "absolute")
subData.m3 <- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=LL.3(),
                  data=subData.1)
subData.m4 <- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=LL.5(),
                  data=subData.1)
subData.m5 <- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=CRS.4c(),
                  data=subData.1)
subData.m6 <- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=BC.4(),
                  data=subData.2)
subData.m7<- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=W1.4(),
                  data=subData.1)
subData.m8<- drm(growthRate ~ Conc, 
                  format(setStrain, trim=TRUE),
                  fct=W2.4(),
                  data=subData.1)
anova(subData.m2, subData.m3)

mselect(subData.m2, list(LL.3(), LL.5(), W1.4(), W2.4(), CRS.4c(), BC.4(), BC.5()))
plot(subData.m6)
summary(subData.m6)
ED(subData.m6, c(50))  

ED_table <- ED(subData.m2,c(50), interval="delta", type = "absolte")
ModelSummary <- summary(subData.m2)
...
```{r plate assay statistics, eval = FALSE, echo=FALSE}

# summary(EC50PlateSum$EC50)
dim(EC50PlateTable)
dim(EC50PlateTable[EC50PlateTable$EC50 > 2 & EC50PlateTable$EC50 < 4, ])
dim(EC50PlateTable[EC50PlateTable$EC50 <= 5, ])
dim(EC50PlateTable)

# # dim(EC50PlateSum)
# 
plateM.1 <- lm(EC50 ~ State + Year ,EC50PlateTable)
car::Anova(plateM.1, type = 2)
# 
# 
# # Conidia assay
# # summary of the EC50 estimation
summary(EC50ConidiaSum$EC50)
dim(EC50ConidialTable)
dim(EC50ConidialTable[EC50ConidialTable$EC50 > 1 & EC50ConidialTable$EC50 < 3.5,])
dim(EC50ConidialTable[EC50ConidialTable$EC50 < 5,])
dim(EC50ConidialTable)

# conidia assay anova
conidiaM.1 <- lm(EC50 ~ State + Year ,EC50ConidialTable, type = 2)
car::Anova(conidiaM.1, type = 2)

# detrmine the difference 
compMethodData <- xyPlConiData %>% tidyr::gather(method, EC50, c(pEC50, cEC50)) %>% 
  select(strainName, method, EC50)
car::Anova(lm(EC50~method, compMethodData))
```
