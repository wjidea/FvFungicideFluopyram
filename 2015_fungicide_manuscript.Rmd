...
#### Table 2
```{r Table 2 hormetic effect,eval=FALSE,echo=FALSE}
...
for (strain in levels(factor(fullPlateDataExclude50$setStrain))) {
  ...
  subData.m3 <- drm(growthRate ~ Conc, 
                  ...
                  fct=LL.4(),
                  data=subData.3)
  modelTest <- mselect(subData.m3, list(LL.3(),BC.4()))
...
rownames(EC50SumHormetic) <- paste(EC50Sum$Strain,EC50Sum$Set,sep = "_")
# EC50SumHormetic$Strain[duplicated(EC50SumHormetic$Strain)]
...
```{r plate assay statistics, eval = FALSE}
